...
const runTasks = async (opts, di) => {
    ...
    try {
...

        {
            ...
            const suffix = (environmentName ? `🚀 ${chalk.cyan(environmentName)}${EOL}${EOL}` : '') + `> 上个版本 ${chalk.yellow(latestVersion)}`;
        ...
        if (version) {
            ...
            if (isCI) {
                log.obtrusive(`> 发布版本 ${chalk.yellow(version)}`);
            }
