...
class GitBase extends Plugin {
    ...
    async getLatestVersion() {
        ...
        if (latestVersion) {
            const tagInfo = await this.exec(`git show -s --format='#Committer#%cn#Committer##Date#%cd#Date#' refs/tags/${latestVersion}`).then(
                stdout => stdout || null,
                () => null
            );
            ...
            if (tagInfo) {
                latestTaggerName = tagInfo.match(/#Committer#(.*?)#Committer#/)[1];
                latestTaggerDate = tagInfo.match(/#Date#(.*?)#Date#/)[1];
                if (latestTaggerDate) {
